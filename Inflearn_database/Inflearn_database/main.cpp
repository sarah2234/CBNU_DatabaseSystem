...
int main()
{
	...
	while (true)
	{
		...
		if (!strcmp(query.c_str(), "quit") || !strcmp(query.c_str(), "exit")) break; // quit 또는 exit을 입력받으면 쿼리 입력 종료
		...
		if (strcmp(query_type.c_str(), "DESC") == 0) // DESC 쿼리일 때
		{
		...
		else if (strcmp(query_type.c_str(), "SELECT") == 0) // SELECT 쿼리일 때
		{
